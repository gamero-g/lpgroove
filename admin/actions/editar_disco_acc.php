...
function backWithErrors(array $errors, array $old, $id): void {
    ...
    header("Location: ../index.php?seccion=editar_disco&id=$id&text=editar disco");
    exit;
}
...
$subioNuevaPortada = $discoArchivos && !empty($discoArchivos['tmp_name']);
if ($subioNuevaPortada && ($discoArchivos['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
    $errors['portada'] = 'La portada nueva es inválida.';
}
...
if ($errors) backWithErrors($errors, $old, $discoId);

try {
    $disco = Disco::filtrarPorId($discoId);
    if (!$disco) {
        $errors['general'] = 'No se encontró el disco.';
        backWithErrors($errors, $old, $discoId);
    }

    if ($subioNuevaPortada) {
        $imagen = Imagen::subirImagen($discoArchivos, '../../img/portadas/');
        Imagen::eliminarImagen("../../img/portadas/" . ($discoDatos['portada_og'] ?? ''));
    } else {
        $imagen = $discoDatos['portada_og'] ?? '';
    }

    $disco->edit(
        $discoDatos['banda'],
        empty($discoDatos['descuento']) ? null : $discoDatos['descuento'],
        $discoDatos['titulo'],
    ...
    if (isset($discoDatos['generos'])) {
        foreach ($discoDatos['generos'] as $genero) {
            Disco::insertDiscoXGenero($disco->getId(), $genero);
        }
    }

} catch (Throwable $error) {
    $errors['general'] = 'No se pudo editar el disco.';
    backWithErrors($errors, $old, $discoId);
}
